...
def _build_madd_args(df: pd.DataFrame) -> list[tuple[str, int, float]]:
    ...
    for row in df.itertuples():
        args.append((f"ts:{row.type}:start", int(row.startDate), float(row.value)))  # type: ignore[arg-type]  # ty: ignore[invalid-argument-type]
        args.append((f"ts:{row.type}:end", int(row.endDate), float(row.value)))  # type: ignore[arg-type]  # ty: ignore[invalid-argument-type]
...
def upload_batch(
    ...
    args = _build_madd_args(df)
    response: list[Any] = rts.madd(args)  # type: ignore[no-untyped-call]
