...
class HealthDataImporter:
    ...
    def etl(
        ...
        **overwrite** :attr:`failures_file` so that :meth:`retry_failed`
        ...
        transform_records(
            records_df, correlations_df[["_record_keys", "correlation_id"]]
        )
    ...
    def retry_failed(self, *, persist_failures: bool = True) -> None:
        ...
        * If **some** failures remain, the file is **overwritten** with
        ...
        transform_records(
            retry_df, correlations_df[[["_record_keys", "correlation_id"]]]
        )
    ...
    def _extract_routes(self, workouts_df: pd.DataFrame) -> pd.DataFrame:
        ...
        route_paths = pd.Series(
            [
                path.lstrip("/")
                for route in workouts_with_route
...
def _upload_type(
    ...
    for group_nr, (group_val, r) in enumerate(g):
        nn = len(r)
        # calculate keys and labels
        base_labels: dict[str, str] = {
            "unit": str(r.iloc[0]["unit"]),
            "identifier": data_type,
            "group": str(r.iloc[0]["group"]),
        }
