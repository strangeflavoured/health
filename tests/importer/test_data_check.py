...
def _category_row(
    value: str = CATEGORY_VALUE_ASLEEP, type_: str = CATEGORY_TYPE, unit: str = ""
) -> dict:
    return {"type": type_, "value": value, "unit": unit}
...
class TestCheckStringValuesAreCategoricalIdentifiers:
    ...
    def test_nan_values_are_filtered_before_check(self) -> None:
        ...
        df = _df(
            _quantity_row(value=float("nan")),  # ty: ignore[invalid-argument-type]
            _quantity_row(value="72"),
        )
        _check_all_string_values_are_categorical_identifiers(df)
...
class TestCheckMissingUnitsAreCategoricalIdentifiers:
    ...
    def test_error_lists_all_offending_types(self) -> None:
        df = _df(
            ...
            _quantity_row(type_=QUANTITY_TYPE_BODY_MASS, value="70", unit=None),  # type: ignore[ty:invalid-argument-type]
        )
        with pytest.raises(DataSanityError) as exc_info:
            _check_all_missing_units_are_categorical_identifiers_or_meta(df)
        msg = str(exc_info.value)
...
class TestCheckUnits:
    ...
    def test_raises_type_error_when_unit_is_nan(self) -> None:
        # Single-unit NaN is treated as valid (ignored) — does NOT raise.
        df = _df(
            _quantity_row(type_=QUANTITY_TYPE_BODY_MASS, value="70", unit=float("nan")),  # ty: ignore[invalid-argument-type]
        )
        with pytest.raises(
            TypeError, match="_check_units expects ArrowStringArray for unique units"
        ):
            _check_units(df)
    ...
    def test_nan_mixed_with_correct_unit_is_multiple_units(self) -> None:
        ...
        df = _df(
            _quantity_row(type_=QUANTITY_TYPE_BODY_MASS, value="70", unit="kg"),
            _quantity_row(type_=QUANTITY_TYPE_BODY_MASS, value="71", unit=float("nan")),  # ty: ignore[invalid-argument-type]
        )
        with pytest.raises(ExceptionGroup) as exc_info:
            _check_units(df)
    ...
    def test_categorical_type_with_empty_unit_passes(self) -> None:
        df = _df(
            _category_row(unit=""),
            _category_row(),
        )
...
class TestCheckExportData:
    ...
    def test_raises_exception_group_collecting_all_failures(self) -> None:
        ...
        df = _df(
            {"type": "HKGhostType", "value": "not-a-number", "unit": "lala"},
        )
        with pytest.raises(ExceptionGroup) as exc_info:
            check_export_data(df)
    ...
    def test_all_leaf_exceptions_are_value_errors(self) -> None:
        df = _df({"type": "HKGhostType", "value": "not-a-number", "unit": "fufu"})
        with pytest.raises(ExceptionGroup) as exc_info:
            check_export_data(df)
        for exc in exc_info.value.exceptions:
            assert isinstance(exc, DataSanityError)
    ...
    def test_aggregates_failures_across_three_distinct_checks(self) -> None:
        ...
        df = _df(
            ...
            _quantity_row(
                type_=QUANTITY_TYPE_BODY_MASS,
                value="70",
                unit=None,  # ty: ignore[invalid-argument-type]
            ),  # check 3
