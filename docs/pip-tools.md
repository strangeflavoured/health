# Day-to-Day: Managing Requirements with pip-tools
...
|--------------------------------------|--------------------------------|
| In `src` or `src` and `src/importer` | `src/requirements.in`          |
| Only `src/importer`                  | `src/importer/requirements.in` |
| Only in docs build                   | `docs/requirements.in`         |
| Dev tooling only                     | `requirements-dev.in`          |
...
# If you only edited src/importer/requirements.in:
pip-compile --generate-hashes --strip-extras --output-file src/importer/requirements.txt src/importer/requirements.in
pip-compile --generate-hashes --strip-extras --output-file docs/requirements.txt docs/requirements.in
...
```
1. src/requirements.in          →  src/requirements.txt
2. src/importer/requirements.in →  src/importer/requirements.txt
   ...
3. docs/requirements.in         →  docs/requirements.txt
4. requirements-dev.in          →  requirements-dev.txt             (independent)
```

Steps 2 and 4 are independent of each other and can be run in any order relative to each other, but both require step 1 to have run first. Step 3 requires steps 1 and 2.
